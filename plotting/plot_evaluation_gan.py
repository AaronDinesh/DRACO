...
def load_memmaps(run_dir: Path):
    ...
    gan_pk = _memmap_without_header(run_dir / "gan_pk.npy", np.float32, k_len)
    return k_vals, target_pk, gan_pk
...
def plot_mean_and_iqr(
    ...
    gan_median = np.median(gan_pk, axis=0)
    target_q25 = np.percentile(target_pk, 25, axis=0)
    target_q75 = np.percentile(target_pk, 75, axis=0)
    gan_q25 = np.percentile(gan_pk, 25, axis=0)
    gan_q75 = np.percentile(gan_pk, 75, axis=0)
...
def _proc_task(sample_idx: int, spectra_row: int):
    ...
    return plot_sample(
        ...
        gan_pk=np.asarray(_GAN_PK[spectra_row]),
        idx=sample_idx,
    ...
    if max_workers == 1:
        for sample_idx, spectra_row in tqdm(
            ...
            plot_sample(
                ...
                gan_pk=np.asarray(gan_pk[spectra_row]),
                idx=sample_idx,
                save_dir=out_dir,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Plot GAN evaluation spectra")
    parser.add_argument(
        "--run-dir",
        type=str,
        required=True,
        help="Path to an evaluation run directory produced by evaluate_gan.py",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--noise-gan",
        action="store_true",
        help="Flag indicating results are from the noise GAN variant",
    )
    parser.add_argument(
        "--num-workers",
        type=int,
...
def main():
    ...
    if args.noise_gan and "noise" not in title_prefix.lower():
        title_prefix = f"{title_prefix} (Noise GAN)"
